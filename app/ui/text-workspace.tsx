...
export default function TextWorkspace({originalText, currentText, onTextInput, mode, onModeChange}: any) {
  ...
  return (
    <div className="flex flex-col w-1/2 text-slate-900">
      <div className="flex flex-col border bg-white/55 backdrop-blur-md rounded-lg h-1/2 pl-2 pr-2 pb-2 mb-1">
        <div className="flex flex-row items-center">
          ...
          <div className="basis-1/3 text-right">
            ...
            <button className="rounded-full hover:bg-white/60 p-2" onClick={editInputText}>
              <Image
                src={editIcon}
                alt="Edit Text"
              ></Image>
        ...
        <form onSubmit={handleSubmit} className="flex flex-col grow">
          <textarea 
            name="textinput"
            className="rounded-lg p-1 text-black text-lg resize-none font-mono h-full bg-white/60 hover:bg-white/80 hover:cursor-text" 
            placeholder="Enter your message here!"
            disabled={originalText != ""}
          ></textarea>
          <input type="submit" 
            className={"bg-slate-200 mt-1 rounded-md p-2 hover:bg-white/60 hover:cursor-pointer" + (currentText != "" ? " hidden" : "")}>
          </input>
        </form>
      </div>
      <div className="flex flex-col border bg-white/55 backdrop-blur-md rounded-lg h-1/2 pl-2 pr-2 pb-2 mb-1">
